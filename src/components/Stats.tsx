...
function Stats() {
  ...
  const habitEntriesHist: HabitEntry[] = useHabitEntryStore(
    (state) => state.habitEntriesHist
  );
  ...
  const calcDayStreak = (entries: HabitEntry[]) => {
    ...
    for (let i = 0; i < sortedArr.length; i++) {
      if (
        !!sortedArr[i].complete &&
        yesterday.getTime() - sortedArr[i].timestamp === i * 86400000
      ) {
        count++;
      } else {
        break;
  ...
  const habitStatsText = (
    createdTimestamp: number | object,
    entries: HabitEntry[]
  ) => {
    const entriesCopy = [...entries];
    if (!entries.length) {
      return "This was created today. Stats will show from tomorrow";
    }
    ...
    const completePercent =
      Math.floor((daysCompleted / daysSinceCreation) * 100) + "%";
    const dayStreak = calcDayStreak(entriesCopy);

    let text = (
      <>
        You have completed this <b>{daysCompleted}</b> time
        {daysCompleted === 1 ? "" : "s"} since starting {daysSinceCreation} days
        ago ({completePercent}) and are on a <b>{dayStreak}</b> day streak
      </>
    );
    ...
    <div className="container">
      {habits &&
        habits.map((habit) => (
          <div
            key={habit.id}
            className="container col-xl-10 col-xxl-8 px-4 py-3 mb-4 habit-not-completed shadow-sm rounded"
          >
            <div className="row align-items-center g-lg-4 py-4">
              ...
              <div className="mb-3">
                {habitStatsText(
                  habit.timestamp,
                  getEntriesHistForHabitId(habit.id)
                )}
              </div>
